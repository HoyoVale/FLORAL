...
export type GatewayCommand =
  ...
  | {
      type: "goal";
      action: "status" | "set" | "continue" | "restart" | "active" | "pause" | "complete" | "blocked" | "clear";
...
export function parseGatewayCommand(text: string): GatewayCommand | undefined {
  ...
  const goalAction = /^\/goal(?:\s+(status|continue|restart|active|pause|complete|blocked|clear))?$/i.exec(trimmed);
  if (goalAction) {
    return {
      type: "goal",
      action: (goalAction[1]?.toLowerCase() || "status") as
        | "status" | "continue" | "restart" | "active" | "pause" | "complete" | "blocked" | "clear",
      objective: undefined,
